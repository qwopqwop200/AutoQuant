...
def auto_scale_block(awq_model,
    ...
    if quant_config.bits is not None:
        def w_quantize_func(p): return pseudo_quantize_tensor(p, quant_config.bits, quant_config.group_size,
                                                              quant_config.zero_point, quant_config.inplace).detach()
    else:
    ...
    def _search_module_scale(block, linears2scale: list, x, kwargs={}):
        ...
        w_max = get_weight_scale(weight, group_size=quant_config.group_size)
        # Clear GPU memory
        del weight
        gc.collect()
