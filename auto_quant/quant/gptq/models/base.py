...
class GPTQConfig(BaseQuantConfig):
    ...
    act_order: bool = field(default=True)
...
class BaseGPTQForCausalLM(BaseQuantForCausalLM):
    layer_type: str = None
    ...
    def __init__(self, model, quant_config, is_quantized):
        ...
        self.quant_config:GPTQConfig = quant_config
        self.search_result = None
                
    @classmethod
    def check_quant_config(self, quant_config: GPTQConfig):
        pass
    ...
    def from_quantized(
        ...
        **kwargs,
        ...
        quant_config = GPTQConfig.from_pretrained(quant_path, **cached_file_kwargs)
        ...
        if not(quant_config.act_order) or post_init:
            model = exllama_post_init(model, quant_config.act_order)
    ...
    def quantize(
        ...
        quantizers = {}
        for i in tqdm(range(len(layers)), desc="GPTQ Quantization"):
            layer = layers[i]
            force_layer_back_to_cpu = False
    ...
    def pack_model(self, quantizers):
        ...
        for name in tqdm(layers, desc="Pack Model"):
            ...
            layer, scale, zero, g_idx = layers[name].to('cpu'), scale.to('cpu'), zero.to('cpu'), g_idx.to('cpu')
            qlayer = ExllamaLinear.from_linear(
                    layer, 
                    self.quant_config.bits, 
                    self.quant_config.group_size,
                    self.quant_config.act_order,
                    False, 
                    scale, 
                    zero,
                    g_idx)
