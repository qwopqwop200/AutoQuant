...
def get_named_module(module, layers=None, name=''):
    if not layers:
    ...
    for name1, child in module.named_children():
        res.update(get_named_module(child, layers=layers, name=name + '.' + name1 if name != '' else name1))
    return res
...
def simple_dispatch_model(model, device_map):
    ...
    for idx, (n, d) in enumerate(cpu_offload_group):
        ...
        _, prev_hook = cpu_offload_with_hook(m, execution_device=main_device, prev_module_hook=prev_hook)
